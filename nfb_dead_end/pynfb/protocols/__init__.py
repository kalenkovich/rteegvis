...
class Protocol:
    ...
    def update_state(self, samples, reward, chunk, chunk_size=1, is_half_time=False):
        ...
        if self.source_signal_id is not None:
            if self.mock_previous == 0:
                mark = self.widget_painter.redraw_state(sample=samples[self.source_signal_id], m_sample=m_sample,
                                                        chunk=chunk)
            ...
            mark = self.widget_painter.redraw_state(sample=samples[0], m_sample=m_sample,
                                                    chunk=chunk)  # if source signal is 'ALL'
        return mark
...
class SourceSpaceProtocol(Protocol):
    def __init__(self, signals, name='SourceSpace', **kwargs):
        kwargs['name'] = name
        super().__init__(signals, **kwargs)
        ...
        self.widget_painter = SourceSpaceProtocolWidgetPainter(show_reward=self.show_reward,
                                                               protocol=self
                                                               )
