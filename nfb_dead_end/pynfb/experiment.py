...
from .protocols import BaselineProtocol, FeedbackProtocol, ThresholdBlinkFeedbackProtocol, VideoProtocol, PsyProtocol, SourceSpaceProtocol
...
class Experiment():
    ...
    def update(self):
        ...
        if chunk is not None and self.main is not None:
            ...
            if self.main.player_panel.start.isChecked():
                # subject update
                mark = self.subject.update_protocol_state(samples=samples, reward=self.reward, chunk=chunk, # ch_names=None,
                                                          chunk_size=chunk.shape[0], is_half_time=is_half_time)
    ...
    def restart(self):
        ...
        for protocol in self.params['vProtocols']:
            ...
            elif protocol['sFb_type'] == 'SourceSpace':
                self.protocols.append(
                    SourceSpaceProtocol(
                        self.signals,
                        **kwargs))
